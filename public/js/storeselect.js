...
const city1 = document.querySelector(".sido");
// 구, 군 나오게 하는 select 태그 변수로 세팅
const city2 = document.querySelector(".gugun");
...
const gugun = (city1Data) => {
    ...
    for(let i = 0; i < hangjungdong.gugun.length; i++) {
        // 전달받은 sido 코드값과 행정동 객체에 있는 sido 코드값하고 일치하면 option 만들어준다.
        if (city1Data == hangjungdong.gugun[i].sido) {
            let city2Tag = document.createElement("option");    // 태그생성
            let city2Text = document.createTextNode(hangjungdong.gugun[i].codeNm);   // 태그에 들어갈 텍스트 생성
            city2Tag.append(city2Text);
            city2Tag.setAttribute("value",hangjungdong.gugun[i].codeNm);     // option의 value값에 지역명을 넣어줌
